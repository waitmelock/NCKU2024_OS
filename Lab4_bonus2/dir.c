...
static int osfs_iterate(struct file *filp, struct dir_context *ctx)
{
    ...
    for(int idx=0;idx<osfs_inode->extent_count;idx++){
        ...
        // Calculate the number of directory entries
        dir_entry_count = osfs_inode->extent[idx].file_offset / sizeof(struct osfs_dir_entry);
        dir_entries = (struct osfs_dir_entry *)dir_data_block;

        // Traverse the directory entries and emit them
...
static int osfs_add_dir_entry(struct inode *dir, uint32_t inode_no, const char *name, size_t name_len)
{
    ...
    parent_inode->extent[idx].file_offset += sizeof(struct osfs_dir_entry);
...
{   
    ...
    osfs_inode->extent[0].file_offset = 0; 
    osfs_inode->i_size = 0;
